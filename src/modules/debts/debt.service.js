...
export const createDebt = async (debtData, userId) => {
    try {
...
export const getDebtsByAccountId = async (accountId, userId) => {
    try {
...
export const updateDebt = async (debtId, debtData, userId) => {
    try {
...
export const getAllDebtsByUserId = async (userId) => {
    try {
        const debts = await prisma.debt.findMany({
            where: { userId },
            orderBy: { createdAt: 'desc' },
        ...
        const totalToPayThisMonth = debts.reduce((total, debt) => {
            ...
            if (debt.isSubscription)
                return total + debt.amountPerMonth;
            if (!debt.totalAmount)
                return total + debt.amountPerMonth;
            const paidInstallments = debt.initialPaidInstallments + (debt.payments?.length || 0);
            const remainingAmount = debt.totalAmount
...
export const markDebtAsPaid = async (debtId, userId) => {
    try {
