...
export const debtSchema = z.object({
  ...
  totalAmount: z.number().positive("El monto total debe ser un número positivo").transform(val => Number(val.toFixed(2))),
  totalInstallments: z.number().int().positive("Las cuotas deben ser un número entero positivo").default(1),
  amountPerMonth: z.number().positive("El monto debe ser un número positivo").transform(val => Number(val.toFixed(2))),
  isSubscription: z.boolean(),
  initialPaidInstallments: z.number().int().min(0, "Las cuotas pagadas no pueden ser negativas").default(0),
  payments: z.array(z.object({
    amount: z.number().positive("El monto del pago debe ser un número positivo").transform(val => Number(val.toFixed(2))),
